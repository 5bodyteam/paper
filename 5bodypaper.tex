...
m_{4}=\frac{[f_{4L}(s,t)+f_{4c}c]\lambda+g_{43}(r,s,t)m_{3}}{d}4t^{2}(s+2)^{2} \\
m_{5}=\frac{[f_{5L}(s)+f_{5c}(s)c]\lambda+g_{53}(r,s)m_{3}}{d}t^{2}(t+2)^{2}(s+t+2)^{2} \\
f_{1L}(s,t)=\frac{t^{2}[(s+2)^{2}(t+2)^{2}[s^{2}+(t+1)(s+t+2)^{2}]-4(s+t+2)^{2}[(s+2)^{2}+s^{2}]]}{d}-1\\
f_{1c}(s,t)=\frac{t^{2}[(s+2)^{2}[(s^{2}(t+2)^{2}+4(s+t+2)^{2}]-(s+t+2)^{2}[(s+2)^{2}(t+2)^{2}+4s^{2}]]}{d}-1\\
...
f_{2L}(s,t)=1+s+\frac{t^{2}[4s^{2}(s+t+2)^{2}(1+s)+4(s+2)^{2}(t+2)^{2}-(t+2)^{2}[s^{2}(1+s)(s+2)^{2}+4(t+1)(s+t+2)^{2}]}{d}\\
f_{2c}(s,t)=\frac{t^{2}[4(s+t+2)^{2}[s^{2}+(t+2)^{2}]-(s+2)^{2}(t+2)^{2}(4+s^{2})]}{d}+1\\
...
To satisfy the equation for $m_3$, we substitute our expressions for $m_1,m_2,m_4,m_5$ to obtain our c. Through this process, we discover that center of mass did not rely on $m_1, m_2, m_3, m_4, m_5, or \lambda$, but that it relies only on the parameters r, s, and t.

\begin{equation}
c(s,t,r)=\frac{s^{2}[\frac{f_{1L}}{(r+s+1)^{2}}+\frac{f_{2L}}{(r+1)^{2}}]-\frac{t^{2}}{d}[\frac{f_{4L}4(s+2)^{2}}{(r-1)^{2}}+\frac{f_{5L}(t+2)^{2}(s+t+2)^{2}}{(t+1-r)^{2}}]-r}{\frac{t^{2}}{d}[\frac{f_{4c}4(s+2)^{2}}{(r-1)^{2}}+\frac{f_{5c}(t+2)^{2}(s+t+2)^{2}}{(t+1-r)^{2}}]-s^{2}[\frac{f_{1c}}{(r+s+1)^{2}}+\frac{f_{2c}}{(r+1)^{2}}]-1}
\end{equation}
...
\end{enumerate}
where $h_{1}=\frac{h_{1n}}{h_{1d}}$, $h_{2}=\frac{h_{2n}}{h_{2d}}$, \\
...
Since the external terms are positive, we can multiply both sides by these terms without changing the inequality.
...
Since $h_{1d} = 17s^{4}+68s^{3}+100s^{2}+64s+16$ is always positive, we can multiply both sides by the denominator without
...
\includegraphics[width=2.7in, height=2.7in]{Lm3RegionSpecialCase.png}
 \caption{ \label{EL}  Region for $L/m_3$ in terms of s. The blue region is the area where central configurations are possible.  }
\end{figure}
\end{center}


\section{General Case Centeral Configuration}

\begin{theorem}\label{thm5}
\emph{Let $0<r<1$, $s>0$, $t>0$, and $q=(-1-s,-1,r,1,1+t)$
with masses $m_{i}=F_{i}L+G_{i}M$ respectively. Then q is a central
configuration if $\frac{-F_{1}}{G_{1}}<\frac{M}{L}<\frac{-F_{4}}{G_{4}}$
and for a given t, the possible configurations lie in the region under
