...
\begin{figure}
\includegraphics[width=6in, height=6in]{r0o.png}
...
We can reorganize the list of permutations and reduce the number of cases once we investigate the inverse permutations and their properties. For a permutation, $\tau$, the inverse permutation, ${\tau}^{-1}$, will be the permutation such that $\tau({\tau}^{-1})=(1,2,3,4,5)$ and ${\tau}^{-1}(\tau)=(1,2,3,4,5)$. We have that the following pairs of permutations are inverses:
$$\tau_1=(2,4,1,5,3) \hspace{20pt} \tau_8=(3,1,5,2,4)$$
$$\tau_2=(2,5,1,3,4) \hspace{20pt} \tau_7=(3,1,4,5,2)$$
$$\tau_3=(2,5,4,1,3) \hspace{20pt} \tau_{13}=(4,1,5,3,2)$$
$$\tau_4=(2,3,5,1,4) \hspace{20pt} \tau_{14}=(4,1,2,5,3)$$
$$\tau_5=(2,3,4,5,1) \hspace{20pt} \tau_{19}=(5,1,2,3,4)$$
$$\tau_6=(2,4,5,3,1) \hspace{20pt} \tau_{20}=(5,1,4,2,3)$$
$$\tau_9=(3,4,5,1,2) \hspace{20pt} \tau_{16}=(4,3,1,5,2)$$
$$\tau_{10}=(3,4,5,1,2) \hspace{20pt} \tau_{15}=(4,5,1,2,3)$$
$$\tau_{11}=(3,4,2,5,1) \hspace{20pt} \tau_{21}=(5,3,1,2,4)$$
$$\tau_{12}=(3,5,4,2,1) \hspace{20pt} \tau_{22}=(5,4,1,3,2)$$
$$\tau_{17}=(4,5,2,3,1) \hspace{20pt} \tau_{24}=(5,3,4,1,2)$$
$$\tau_{18}=(4,3,5,2,1) \hspace{20pt} \tau_{23}=(5,4,2,1,3)$$

Now we must prove that knowing the inverse permutations is useful.

\begin{lemma}
If we have $\tau$ and ${\tau}^{-1}$, then we can solve for $\lambda$ and $\lambda_\tau$ as well as $\lambda$ and $\lambda_{{\tau}^{-1}}$. They will have the relationship that $\lambda=\lambda_{{\tau}^{-1}$ and $\lambda_\tau=\lambda$. Additionally, if $q$ works for $\tau$ then $q$ will also work for ${\tau}^{-1}$
\end{lemma}
